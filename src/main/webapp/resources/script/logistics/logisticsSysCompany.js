    ...
    var coun= {   
    		...
    		function operate(cellValue,options,rowObject){
    		  	var template='';
    			if(options.rowId == 1){
        			 template='<button class="btn btn-primary btn-sm btn-setchannel channelSetting mr5 "   onclick="editPage(' + rowObject.logisticsId+ ')">设置</ button>'+
        		   	'<button  class="btn btn-default btn-sm btn-setchannel channelSetting mr5 " onclick="copyLogistics(' +  rowObject.logisticsId+ ',\''+rowObject.name+'\')">复制</button>';
    			}else{
    				template='<button class="btn btn-primary btn-sm btn-setchannel channelSetting mr5 "   onclick="editPage(' + rowObject.logisticsId+ ')">设置</ button>'+
        		   	'<button  class="btn btn-default btn-sm btn-setchannel channelSetting mr5 " onclick="copyLogistics(' +  rowObject.logisticsId+ ',\''+rowObject.name+'\')">复制</button>'+
        		   	'<button  class="btn btn-default btn-sm text-danger doDelete" onclick="del(' +  rowObject.logisticsId+ ',\''+rowObject.name+'\')">删除</button></div>	';
    			}
    				return template;
    		   };
		   function status(cellValue,options,rowObject){
   		   	var template='<div class="switch switch-small"><input type="checkbox" checked />	 </div>   	';
   				return template;
   		   };
   		 //平台认可物流渠道
