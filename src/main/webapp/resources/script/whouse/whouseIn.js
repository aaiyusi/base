    ...
    url : ctx.path +'/api/whouseIn/queryList.do',
    mtype:"POST",
    datatype : "json",
    height: top.getFrameHeight() - $("#buttonDiv").outerHeight() - $("#serachDiv").outerHeight() - 125,
    colNames : ['id','批次编号','仓库','备注','SKU个数','入库时间','操作人','操作'],
    ...
    //下拉框
    $.ajax({  
        url:  ctx.path +"/api/whouseIn/selectWhouse.do",    //后台webservice里的方法名称  
        type: "post",  
        dataType: "json",  
		...
		if($("#whouseAddSelect").children('option:selected').val() != "" && $(this).val() !=""){
	  		$.ajax({  //判断sku是否存在仓库
                ...
                success: function (data) {  
                	var jsonObj =data.data;  
                	if(data.rs == 1){
                		if(data.data.length == 1){//已存在商品仓位，取当前仓位
                			if("space_code" in jsonObj[0]){//如果有仓位则继续带出
                				var optionstring = "";  
                				for (var j = 0; j < jsonObj.length; j++) {  
        	            		 ...
        	            		 */
                    	  		$.ajax({  
                                    url:  ctx.path +"/api/whouseIn/querySpaceByWhouse.do",    //根据仓库取所有仓位
                                    data : {
                                    	whouseId : $("#whouseAddSelect").children('option:selected').val(),
                                    	sku : $('#InSKU').val()
                        			},
                                    ...
                                    success: function (data) {  
                                    	if(data.rs == 1){
                                    			...
                                    			$("#spaceId").html("<option value='' >-选择仓位-</option>"+optionstring);  
                                    	}
                                    },  
                                    error: function (msg) {  
                                    	top.toastr.error(msg);
                	    	...
                	    	//验证SKU是否存在
                	        $.ajax({  
                	            url:  ctx.path +"/api/whouseIn/queryBySku.do",    //后台webservice里的方法名称  
                	            data : {
                					sku : $("#InSKU").val()
                				},
                	            type: "post",  
  ...
  //仓库下拉选择联动库位
    ...
    //联想下拉
    $.ajax({  
        url:  ctx.path +"/api/whouseOut/querySkuByWhouseId.do",    //后台webservice里的方法名称  
        type: "post",  
        data:{
        	whouseId : $(this).val()
        },
    ...
    $("#childAdd").bind("click",function(){
    	...
    	if(isFlag == false){
    		top.toastr.error("SKU编号和入库数量及仓位不能为空或格式不正确");
    		return false;
    	}
    	if(!checkUtil.isInt($("#InCount").val())){
    		$("#InCount").focus(); 
    		top.toastr.error("入库数量格式不正确");
    		return false;
    	}
    	if($("#price").val() != "" && !checkUtil.isMoneyValue($("#price").val())){
    		$("#price").focus(); 
    		top.toastr.error("入库金额格式不正确");
    		return false;
    	}
    	//验证SKU是否存在
        $.ajax({  
...
//					if(entity.saleState == 2 || entity.saleState == 3){//商品状态是正常销售或者待开发
...
//		                    $("#jqAddGrid").jqGrid('setSelection',newrowid);//选中新增行
					...
					/*}else{
						top.toastr.error("此库存SKU【"+entity.sku+"】状态不是【正常销售或等待开发】");
					}*/
            	}else{	
            		top.toastr.error("您输入的sku不存在");
            		return false;
    ...
    function openAddPage(){
    	chlidGrid = $("#jqAddGrid").jqGrid({
	        ...
	        colNames : ['id','spaceId','缩略图','库存SKU编号','中文名称','仓位','变动前库存','入库数量','单价','金额','操作'],
	        colModel : [{name : 'detailId',index:'detailId',hidden:true},
	                    		...
	                    		{name : 'price',editable: true,index:'price',width : 100},
	                    		{name : 'totalPrice',index:'totalPrice',width : 100},
	                    		{name : 'make' ,width : 140,
	                    			formatter : function(cellvalue, options, rowObject){
	        ...
	        onSelectRow : function(id) {
	            ...
	            edit = true;
	             ...
	             jQuery('#jqAddGrid').jqGrid('restoreRow', lastsel);//重置行状态
	              ...
	              rowData.totalPrice = rowData.count * rowData.price;
	              jQuery("#jqAddGrid").jqGrid('setCell', lastsel,"totalPrice",rowData.totalPrice);//修改行内容
       ...
       //表单自适应
       function reSizeAdd(){
    	   	var width = $('.jqGrid_Addwrapper').width();
    	    $('#jqAddGrid').setGridWidth(width);
    	    var newHeight=top.getFrameHeight()  - 75;
    	    $("#jqAddGrid").jqGrid('setGridHeight',newHeight);
       }
    ...
    if(!checkUtil.isNullValue(obj)){
    	...
    	top.toastr.error("请至少输入一个入库的SKU");
    }
    },
    content: $('#dataAdd'),
    ...
    function whouseInSave(){
    	...
    	var flag = true;
    	 ...
    	 var obj = $("#jqAddGrid").jqGrid("getRowData");
    	    jQuery(obj).each(function(key){//遍历grid行
    	    	...
    	    	if(obj[key].price != "" && !checkUtil.isMoneyValue(obj[key].price)){
    	    		top.toastr.error("入库单价格式不正确");
    	    		flag = false;
            		return false;
    	    	}
    	    });
    	...
    	whouseStorage.skuCount = whouseInChild.length;
    	    ...
    	    param.wsdList = whouseInChild;//子表list插入集合
    	    param.whouseStorage = whouseStorage;//主表插入集合
    	    
        		
    	    
            $.ajax({  
                url:  ctx.path +"/api/whouseIn/addStrorage.do",    //后台webservice里的方法名称  
                type: "post",  
                dataType: "json",  
                layerMaskFlag:true,
                ...
                success: function (data) {  
                	if(data.rs == 1){
                		top.toastr.success("新增入库批次成功");
                	}else{
                		top.toastr.error("新增入库批次失败");
                	}
                	 //清空内容
                    FormUtil.resetForm("whouseInChildForm");
                    jQuery("#jqAddGrid").jqGrid("clearGridData");
     ...
     */
      function detialWhouseChild(rowId){
      	...
      	//主表赋值
        $.ajax({  
            url:  ctx.path +"/api/whouseIn/queryDetialHeadByStorageId.do",    //后台webservice里的方法名称  
            type: "post",  
            data:{
            	storageId :rowData.storage_id
            },
        ...
        var detialGrid = $("#jqDetialGrid").jqGrid({
            url : ctx.path +'/api/whouseIn/queryDetialByStorageId.do?storageId='+rowData.storage_id,
            mtype:"POST",
            datatype : "json",
            height: top.getFrameHeight() - $("#buttonDiv").outerHeight() - $("#serachDiv").outerHeight() - 110,
            colNames : ['id','缩略图','库存SKU编号','中文名称','仓位','变动前库存','入库数量','单价','金额'],
        ...
        detialGrid.jqGrid("setGridParam", {url:ctx.path +'/api/whouseIn/queryDetialByStorageId.do?storageId='+rowData.storage_id});
